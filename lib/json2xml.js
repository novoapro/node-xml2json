...
ToXml.prototype.parse = function (obj) {
    ...
    keys.filter(function (c) {
        return c.startsWith(self.options.attributePrefix)
    }).forEach(function (attribute) {
        var value = obj[attribute];
        self.addAttr(attribute.replace(self.options.attributePrefix, ""), value);
    });
            ...
            if (type == 'string' || type == 'number' || type == 'boolean' || isArray) {
                ...
                it.forEach(function (subVal) {
                    if (typeof (subVal) != 'object') {
                        if (attribute == self.options.valueKey) {
                            self.addTextContent(subVal);
                        } else {
                            self.openTag(attribute);
                            self.addTextContent(subVal);
                            self.closeTag(attribute);
                        }
                    }
                });
            }
